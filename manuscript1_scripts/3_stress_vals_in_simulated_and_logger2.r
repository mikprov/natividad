...
stress4 <- stress4 %>% 
  ...
  mutate(regtype="version2")
colnames(stress4)[colnames(stress4)=="stress4_dev"] <- "daily_stress"

stress_version2 <- bind_rows(stress1,stress2,stress3,stress4,.id=NULL)
stress_version2$datatype <- rep("mSST",nrow(stress_version2))

# ---
# calc stress on logger data
# ---
...
# ---
# combine logger stress and mSST stress
# ---
logger_mSST_version2 <- bind_rows(logger_stress,stress_version2)

# ---
# integrate daily_stress values
# ---
method <- unique(logger_mSST_version2$method)
datatypes <- unique(logger_mSST_version2$datatype)
stress_vals <- expand.grid(site_vector,method,datatypes)
colnames(stress_vals) <- c("Site","method","datatype")
...
for(d in 1:length(datatypes)){ # for mSST and logger data types
  ...
  for(m in 1:length(method)){ # step through stress methods (1-4)
    ...
    for(s in 1:length(site_vector)){ # and for each site
      ...
      yy <- logger_mSST_version2[logger_mSST_version2$method==method[m] &
                                 logger_mSST_version2$datatype==datatypes[d] &
                                 logger_mSST_version2$Site==site_vector[s] ,]$daily_stress
      
      yy <- yy[!is.na(yy)]
      xx <- seq(from=1,to=length(yy),by=1)
      stress_vals[stress_vals$method==method[m] &
...
} #close datatypes loop
rm(m,d,s,yy,xx) 
  
  
# ---
# normalize integral values across sites (mSST & logger)
# ---
stress_vals <- stress_vals[!stress_vals$Site=="LaJolla",]

for(d in 1:length(datatypes)){ #for mSST & logger datatypes
  
  for(m in 1:length(method)){ #step through each stress method
    
    # min stress integral value among sites for that regression type and stress method
    min_int <- min(stress_vals[stress_vals$method==method[m] &
                               stress_vals$datatype==datatypes[d],]$integral)
    
    # max stress integral value among sites for that regression type and stress method
    max_int <- max(stress_vals[stress_vals$method==method[m] &
                               stress_vals$datatype==datatypes[d],]$integral)
    
    # relative integral value
    stress_vals[stress_vals$method==method[m] &
                stress_vals$datatype==datatypes[d],]$integral_relative <-
      
      ((stress_vals[stress_vals$method==method[m] &
                    stress_vals$datatype==datatypes[d],]$integral) - min_int) / (max_int - min_int)
  }
}
...
# prep df for adding regression info

stress_vals_plotting <- stress_vals %>% 
  select(Site,method,datatype,integral_relative) %>%
  spread(key=datatype,value=integral_relative) %>%
  mutate(r2=rep(NA)) %>% 
  mutate(yint=rep(NA)) %>%
  mutate(slope=rep(NA))
...
for(m in 1:length(method)){
  ...
  temp <- stress_vals_plotting[stress_vals_plotting$method==method[m],]
  temp_lm <- lm(temp$mSST ~ temp$logger)
  
  stress_vals_plotting[stress_vals_plotting$method==method[m],]$r2 <-
    rep(summary(temp_lm)$r.squared)
  
  stress_vals_plotting[stress_vals_plotting$method==method[m],]$yint <-
    rep(summary(temp_lm)$coefficient[1])
  
  stress_vals_plotting[stress_vals_plotting$method==method[m],]$slope <-
    rep(summary(temp_lm)$coefficient[2])} #close loop



# ---
# plot version 2 stress values in logger and mSST data
# ---
my_plotlabels = paste0(letters[1:4], ') ')

stress_vals_plotting$letterlabel <- 
  paste(rep(my_plotlabels,times=16),stress_vals_plotting$method,"   r2=",round(stress_vals_plotting$r2,digits=2),sep="")


version2 <- ggplot(data=stress_vals_plotting) +
  facet_wrap(vars(letterlabel)) +
  geom_point(aes(x=logger,y=mSST)) +
  geom_abline(intercept = 0, slope=1) +
  geom_abline(data=stress_vals_plotting,aes(slope=slope,intercept = yint),linetype="dashed") +
  geom_text_repel(aes(x=logger,y=mSST,label=Site),
                  size=3,color="grey") +
  theme_bw() +
  ylab("mSST") + xlab("logger") 
  #ggtitle("Linear weak sites, Log strong sites") 
version2

jpeg(filename = "C:/Users/Mikaela/Documents/GitHub/natividad/manuscript2_figs/fig2abcd.jpeg", res=300, height = 5, width = 6, units="in")
version2
dev.off()
rm(version2)
