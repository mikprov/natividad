...
All the 'reserve' sites in El Rosario:
...
And 'reserve' sites in Isla Natividad:
...
I assume: in 2016-2018 any abalone species missing from monitoring data was actually searched for in all transects at the sites of interest (reserve sites in Isla Natividad and El Rosario) but no abalone were observed. 
```{r echo=FALSE, error=FALSE, message=FALSE, warning=FALSE}
...
for(i in 1:length(mdf$Year)){
  a <- mdf[i,]
  mdf[i,]$density <- a$sum.per.site / meta[meta$Site== a$Site & meta$Year== a$Year,]$totalTransect_site_yr
...
mdf %>% group_by(Site,Year) %>% summarise(total_transects = sum(numtransects))
```
